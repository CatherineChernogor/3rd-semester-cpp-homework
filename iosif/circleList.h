...
public:
    ...
    template<class>
    friend ostream &operator<<(ostream &out, CircleList &list);
};

template<class T>
CircleListElem<T>::CircleListElem(const T &adata, CircleListElem<T> *anext) {
    data = adata;
    next = anext;
}

template<class T>
CircleListElem<T> *CircleList<T>::getStart() {
    return start;
}
...
CircleList<T>::~CircleList<T>() {
    ...
    CircleListElem<T> *ptr = start;
    //for (int i = 0; ptr->getNext() != 0 || ptr->getNext()!=&start; i++) {
    for (int i = 0; ptr->next != 0 || ptr->next != start; i++) {
        deleteFirst();
        /*
        if (ptr->next == start) {
...
/*
    cout << ptr->data << "out list " << ptr << endl << endl;
    cout << ptr->data << "deleted " << ptr << endl;
...
/*
    while (start) {
        ...
        deleteFirst();
        /*
         * удаляем всегда старт указатель,
...
void CircleList<T>::deleteFirst() {
    ...
    if (start) {
        ...
        //cout << "deleteFirst " << start->data << ' ' << start << endl;
        delete start;
        start = temp;
...
template<class T>
void CircleList<T>::deleteAfter(CircleListElem<T> *ptr) {
    if (ptr && ptr->next) {
...
template<class T>
void CircleList<T>::insertAfter(CircleListElem<T> *ptr, const T &data) {
    if (ptr) {
        CircleListElem<T> *temp = ptr->next;
...
template<class T>
ostream &operator<<(ostream &out, CircleList<T> &list) {
